@extends('layouts.app')

@section('content')
<div class="container">
    <div class="row justify-content-center">
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">PRODUCTOS AGREGADOS AL CARRITO</div>

                <div class="card-body">
                    @forelse ($cartQuery as $product)
                    <div class="card" style="width: 10rem;">
                            <img class="card-img-top" src="data:image/png;base64," alt="Card image cap">
                            <div class="card-body">
                              <h5 class="card-title">{{ $product->variant_name }}</h5>
                              <p class="card-text">{{ $product->variant_description }}</p>
                              <p class="card-text">{{ $product->variant_prices_price }}</p>
                              {!! Form::open(['url'=>route('cart.remove')]) !!}
                              {!! Form::hidden('id', $product->variant_id) !!}
                              {!! Form::submit('REMOVER DEL CARRITO', ['class' => 'btn btn-primary']) !!}
                              {!! Form::close() !!}
                            </div>
                    </div>
                    @empty
                        No hay productos agregados
                    @endforelse

                </div>
            </div>
        </div>
    </div>
</div>
@endsection
